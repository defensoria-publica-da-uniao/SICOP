...
//return $function_ret;
    ...
    function checkCarrinho($idUsuario)
    {
        ...
        return $retorno = mssql_num_rows($consulta);
    }
    ...
    function comboListarArray($tabela, $id, $nome, $idSelecionado = null)
    {

        $sql = $this->listaDados($tabela);
        $id_value = $id;
        $nome_select = $nome;
        $option = '';
        while ($dados = mssql_fetch_array($sql)) {
            ...
            if ($idSelecionado <> null && $idSelecionado <> 'inicio') {
                ...
                //Varre array enviado para montar o select com SELECTED
                for ($i = 0; $i < count($idSelecionado); $i++) {
                    if ($idSelecionado[$i] == $id) {
                        $flag = 1;
                        $option = $option . "<option value='$id'selected>$nome</option>";
                    }
                }
                //Se a variavel auxiliar for igual a 0, monta option sem SELETED
                if ($flag == 0) {
                    $option = $option . "<option value='$id'>$nome</option>";
                }
            } else {
